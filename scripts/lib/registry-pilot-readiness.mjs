...
export const collectRegistryPilotChecks = (rootDirectory) => {
  ...
  const registryFeatureSources = [
    ...
    'src/features/registry/RegistryCollections.tsx',
  ...
  return {
    ...
    cartulary_authority: {
      passed: forbiddenRegistryDependencies.every((marker) => !registryFeatureSources.includes(marker))
        && hasAll(projectionService, ["collection(db, 'registries', registryId, 'items')"]),
      evidence: ['src/features/registry', 'src/services/projections.ts'],
    },
    route_contract: {
