...
const scenes = [
  { name: 'accueil', path: '/' },
  { name: 'accueil-menu', path: '/', open: '.public-menu-trigger' },
  ...['cover', 'media', 'reference', 'condition', 'value', 'publication'].map((anchor) => ({ name: `demo-${anchor}`, path: `${DEMO}#${anchor}`, demo: true })),
  { name: 'demo-preuves', path: `${DEMO}#cover`, demo: true, open: '.page-tabs__audit' },
  { name: 'demo-a-faire', path: `${DEMO}#cover`, demo: true, open: '.todo-trigger' },
  ...['accessibilite', 'conditions', 'confidentialite', 'service'].map((page) => ({ name: page, path: `/${page}` })),
  { name: 'sign-in', path: '/account/sign-in' },
  { name: 'not-found', path: '/page-inexistante' },
];
const viewports = [
...
// Assertions hors axe, évaluées dans la page : débordement, lien d'évitement au focus, h1 de couverture, onglet actif visible, langue, graphie.
const CHECKS = `(() => {
  ...
  return { innerWidth, scrollWidth: document.documentElement.scrollWidth, lang: document.documentElement.lang, languageToggles: document.querySelectorAll('.language-toggle').length,
...
const evaluateChecks = (scene, viewport, values) => {
  const checks = [
    { name: 'scrollWidth ≤ innerWidth', ok: values.scrollWidth <= values.innerWidth, detail: `${values.scrollWidth}/${values.innerWidth}` },
...
const report = { date: new Date().toISOString(), base, chrome: chromePath, axe: JSON.parse(readFileSync(new URL('../node_modules/axe-core/package.json', import.meta.url), 'utf8')).version, tags: TAGS, scenes: [] };
...
try {
  ...
  for (const viewport of viewports) {
    for (const scene of scenes) {
      ...
      const entry = { scene: scene.name, viewport: viewport.name, url: scene.path, ready, opened, violations: [], blocking: 0, allowed: 0, other: 0, checks: [] };
      if (scene.open && !opened) { entry.skipped = 'déclencheur absent à cette largeur'; report.scenes.push(entry); await browser.send('Target.closeTarget', { targetId }); continue; }
      await evaluate(axeSource);
      const results = await evaluate(`axe.configure({ locale: ${axeLocale} }); axe.run(document, { runOnly: { type: 'tag', values: ${JSON.stringify(TAGS)} }, resultTypes: ['violations'] }).then((r) => ({ violations: r.violations, passes: r.passes.length, incomplete: r.incomplete.map((i) => i.id) }))`);
      entry.passes = results.passes; entry.incomplete = results.incomplete;
      ...
      entry.checks = evaluateChecks(scene, viewport, await evaluate(CHECKS));
      report.scenes.push(entry);
      await browser.send('Target.closeTarget', { targetId });
      console.error(`${viewport.name} ${scene.name} : ${entry.blocking} bloquante(s), ${entry.allowed} tolérée(s), ${entry.other} mineure(s), assertions ${entry.checks.filter((c) => !c.ok).length} en échec`);
    }
  }
} finally {
...
report.summary = { blocking: blockingTotal, failedChecks, unusedAllowlist: allowlist.filter((entry) => !usedEntries.has(entry)).map((entry) => `${entry.rule} ${entry.target ?? ''}`.trim()) };
...
const rows = report.scenes.map((scene) => `| ${scene.scene} | ${scene.viewport} | ${scene.skipped ? '—' : scene.blocking} | ${scene.skipped ? '—' : scene.allowed} | ${scene.skipped ? '—' : scene.other} | ${scene.skipped ? scene.skipped : scene.checks.filter((check) => !check.ok).length} |`);
...
writeFileSync(join(outDir, `${day}.md`), [`# Audit d'accessibilité — ${day}`, '', `axe-core ${report.axe}, étiquettes ${TAGS.join(', ')} ; Chrome headless (${chromePath}), réseau coupé hors 127.0.0.1 ; ${scenes.length} scènes × ${viewports.length} fenêtres.`, '',
  `**Résultat : ${blockingTotal === 0 && failedChecks.length === 0 ? 'sans erreur' : `${blockingTotal} violation(s) bloquante(s), ${failedChecks.length} assertion(s) en échec`}.**`, '',
  '| Scène | Fenêtre | Bloquantes (serious/critical) | Tolérées (liste d’exceptions) | Mineures (moderate/minor) | Assertions en échec |', '|---|---|---|---|---|---|', ...rows, '',
...
console.error(`Relevé : ${join(outDir, `${day}.json`)} — ${blockingTotal} violation(s) bloquante(s), ${failedChecks.length} assertion(s) en échec.`);
