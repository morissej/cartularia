...
const state = vi.hoisted(() => ({ items: [] as any[], collections: [] as any[], snapshots: [] as any[], observe: vi.fn(), unsubscribe: vi.fn() }));
vi.mock('../../src/services/registryValuation.ts', () => ({ observeRegistryValuationSnapshots: (...args: any[]) => state.observe(...args) }));
...
const registry = { id: 'reg_test', organizationId: 'org_test', name: 'Registre test' } as any;
beforeEach(() => {
  vi.clearAllMocks();
  window.history.replaceState(null, '', '/registry/reg_test/compare?items=a,b');
  state.items = ['a','b','c'].map(id => ({ cartularyId: id, displayTitle: `Objet ${id}`, collectionId: 'col_one', collectionIds: ['col_one'], assetType: 'watch', projectionStatus: 'active', possessionStatus: 'in_possession', lifecycleStatus: 'active', completenessLevel: 'documented', sourceRevision: 1 }));
...
describe('Jams 27 et 29 : valeurs privées dans les vues du Registre', () => {
  it('calcule le total couvert de la Collection sans inclure un objet extérieur et signale la partie manquante', async () => {
    ...
    expect(value.textContent).toContain('2026-09-26');
