...
describe('résumé de publication en lecture seule', () => {
  it('rend les quatre destinations, une seule table et aucune commande d’édition', () => {
    ...
    expect(countOf('Publiez votre objet dans une Collection')).toBe('14 contenus');
    ...
    expect(columnHeaders(tables[0])).toEqual(['Contenu', 'Mini-site', 'Collection', 'Le Cercle', 'Rapport PDF']);
    ...
    expect(within(tables[0]).getAllByText('Inclus')).toHaveLength(14 + 14 + 20 + 23);
    ...
    expect(screen.getByText(/Collection de démonstration : Les cinq icônes — publication non simulée/)).toBeTruthy();
  });
  ...
  it('reflète la sélection réelle et non le catalogue entier', () => {
    ...
    expect(countOf('Publiez votre objet dans une Collection')).toBe('0 contenu');
  ...
  it('n’affiche un lien de mini-site que lorsque la publication réelle est constatée', () => {
    ...
    const table = screen.getByRole('table');
    expect(within(table).getAllByText('Inclus')).toHaveLength(8 + 14 + 20 + 23);
    ...
    // Collection et Cercle restent « ce qui serait publié » : une seule vérité par destination.
    expect(within(articleOf('Publiez votre objet dans une Collection')).getByText(/Ce qui serait publié : 14 contenus/)).toBeTruthy();
  ...
  it('n’annonce aucun compte quand la publication est constatée mais la liste des blocs en ligne inconnue', () => {
    ...
    expect(within(screen.getByRole('table')).getAllByText('Inclus')).toHaveLength(14 + 14 + 20 + 23);
  });
  ...
  it('traduit les en-têtes et les textes contextuels en anglais', () => {
    ...
    expect(columnHeaders(screen.getByRole('table'))).toEqual(['Content', 'Website', 'Collection', 'The Circle', 'PDF report']);
    ...
    expect(within(screen.getByRole('table')).getAllByText('Included')).toHaveLength(71);
