...
2. Décisions en attente, tranchées le 16 septembre (journal `2026-09-16-execution-v7.md` § 2) : activer une version automobile dans le manifeste (V7 : ne rien faire — le statut n’est lu ni par le client ni par le serveur, la création automobile se fait sur la dernière version publiée `car@1.2.0` par la règle `latest` ; publier `car@1.3.0` active puis seeder la production serait une décision éditoriale ADR-005, non retenue ; verrou `tests/hygiene-v7.test.mjs`) ; sortir la correspondance des codes publics de `cartularyIds.ts` (V7 : retirée, `/watch-website` ne dépend que de `publications/{code}` et n’écoute plus les rappels de l’objet actif ; tests `interface-state`, contrat ADR-029, `hygiene-v7`) ; supprimer ou archiver les copies « 2 » (compte réel le 16 septembre : 375 copies sources hors `dist/`, dont 124 sous `public/assets/IWC/derivatives/` et 128 livrées en production par le build ; V7 : plus jamais livrées, par un greffon de build et `hosting.ignore` ; archivage hors iCloud par Jérôme) ; ~~remonter le pilote IWC vers `watch@1.6.0` avec `schema:upgrade`~~ (fait le 9 septembre, journal V1 § 10 : IWC révision 8, Rolex révision 16 ; constaté en V7, seuls la skill et la mémoire étaient périmées).
