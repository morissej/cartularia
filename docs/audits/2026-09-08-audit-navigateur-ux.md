...
Bilan avant V7 (26 constats) : 20 clos, 3 clos partiellement (A2, D6, D7), 3 ouverts réservés à V7 — B2 requalifié par les commits C1/C2, B4 reporté (V8), B5 en attente du commit C4 (polices, D1).
...
| B5 | ouvert : trois polices Google (`index.html:13-15`, quatre requêtes tierces, 198 ko) | V7 · commit C4 (polices hébergées avec le site, D1) en attente du dépôt des trois `.woff2` + `OFL.txt` (J1) | à recetter après C4 : 0 requête `fonts.g*`, trois `.woff2` même origine, `document.fonts.size === 3`, aucun décalage | à recetter après C4 : `h1` en Newsreader, console sans rapport CSP | C4 puis déploiement groupé (D16) ; sans C4, décision explicite (M11) |
