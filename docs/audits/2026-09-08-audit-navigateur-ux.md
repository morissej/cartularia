...
Statut de chaque constat au 16 septembre 2026 (avant V7) et recette V7 : § H, en fin de document (les tableaux A-G restent le relevé daté du 8 septembre).
...
Mis à jour le 16 septembre 2026 — état au 16 septembre 2026 (avant V7, colonne 2) ; les deux colonnes de recette seront remplies après le déploiement V7 (R1 : assistant, visiteur anonyme, navigateur intégré, 1 440 × 900 et 390 × 844 émulé ; R2 : Jérôme, propriétaire dans Chrome et téléphone réel), commit C6.
Sources : `docs/audits/2026-09-16-execution-v7.md` (§ 4 : mesures, requalification de B2, partition pour B4), journaux V0-V6, relevés `docs/audits/perf/2026-09-16.md` et `docs/audits/a11y/2026-09-16.md`, scratchpad `v7/statut-49-constats.md` (preuves détaillées, `fichier:ligne`).
Bilan avant V7 (26 constats) : 20 clos, 3 clos partiellement (A2, D6, D7), 3 ouverts réservés à V7 — B2 requalifié par les commits C1/C2, B4 reporté (V8), B5 clos par le commit C4 (polices hébergées avec le site, D1, 16 septembre) : 21 clos, 3 partiels, B2 requalifié, B4 reporté.
...
|---|---|---|---|---|---|
| A1 | clos | V1 · `interfaceState.ts:37`, `RootPage.tsx:12` ; journal V1 § 2 (quatre pages en production) ; audit axe V6 : quatre scènes titrées, relevé a11y du 16 septembre | à recetter : pied de page et formulaire de contact → quatre pages titrées, à 1 440 et 390 | — | — |
| A2 | clos partiellement | V0 (requalifié) · journal V0 § 1 (App Check 200 en production, 403 propre au navigateur automatisé) ; résidu vérifié : `firebase.ts:25-30` initialise App Check sur toute surface, aucun message `failed-precondition` | à recetter (indicatif) : console sans `AppCheck: 403` sur `/`, la démo, le Registre démo | à recetter : Chrome réel, console ; publication et invitation aboutissent | résidu reporté (V8, D14) : App Check après connexion seulement, candidat avec « retirer Firestore/App Check de la démo publique » |
| A3 | clos | V2 · P2 `DEMO-ROL-124060` publié (révision 4, sceau `S-05BB8C0F`) ; `App.tsx:739,1226` ; `demo-account.test.mjs:303` ; recette anonyme V4 § 6 | à recetter : projection de partage → mini-site quatre pages ; « Voir le mini-site » sur la Submariner seulement ; `DEMO-AP-15510ST` → « Publication absente ou révoquée » ; idem à 390 | — | — |
| A4 | clos | V2 · `registryReturn.ts:33-45`, `App.tsx:638` ; `demo-account.test.mjs`, `tests/ui/registry-return.test.ts` ; production V2 § 5 | à recetter : démo → « Retour au Registre démo » → écran de connexion | — | limite connue (V2 § 3) : propriétaire connecté → « Contexte non autorisé » + « Choisir un autre Registre » |
| A5 | clos | V2 · `registryReturn.ts` (`signedOutRegistryLinks`) ; production V2 § 5 | à recetter : `/community` et `/collection-website` sans session → les deux issues | — | — |
| B1 | clos | V3 · journal V3 § 13.2 (accueil démo 403 ko même origine, 0 vue 360°) ; contrat K6 (`SpinSequence` lazy), `media-preloading`, `presentation-derivatives` | à recetter : `#cover` < 1 Mo, aucune `spin-XX` avant « Ouvrir la séquence » ; idem à 390 | — | — |
| B2 | ouvert avant V7 ; **requalifié par V7 (C1/C2) : nombre de fichiers clos** — 94 → 60 morceaux JS, 0 icône livrée seule (un `icons-*.js`), étape Registre à 0 JS réseau depuis la page de connexion (`docs/audits/perf/2026-09-16.md`) ; durée d'ouverture imputée au second document, à App Check et aux canaux Firestore Listen (27, V3), non au découpage (HTTP/2 : ± 10 ms) | V7 · `vite.config.ts` (groupes react/icons), `src/features/public/registryPreload.ts` ; `tests/performance-v7.test.mjs`, `verify:v7` (`measure:pf0`, `measure:surfaces --check`) | à recetter (session démo) : attendre ≥ 3 s sur la page de connexion avant le clic démo (le préchargement part à ≈ 1,8 s), puis 0 `/assets/*.js` demandé au réseau à l'étape Registre à chaud | à recetter : mesure « avant » (R0, avant le déploiement) puis « après » (R2) : chrono clic → catalogue, JS réseau, canaux Listen, `enterprise.js` | durée : décision après R0/R2 ; V-B4 reporté (V8) |
| B3 | clos | V3 · K5 (`registryThumbnail.ts`, `RegistryItems.tsx`) ; V3 § 13.2 (cinq vignettes, 0 Storage) ; contrat K5, `registry-thumbnail.test.mjs` | à recetter : cinq vignettes au catalogue démo | à recetter : IWC et Rolex avec vignette, 0 requête `firebasestorage` | — |
| B4 | ouvert avant V7 ; **reporté (V8, D4)** — aucun routeur client (`RootPage.tsx:19`), trois documents accueil → démo → Registre | V7 · ticket d'entrée = partition mesurée des surfaces (journal V7 § 4) | — (constater l'écran vide entre deux documents, jugé acceptable à court terme) | — | V8 : option minimale « famille publique + Registre » ou routeur unique, décision après la mesure de production de B2 |
| B5 | **clos (C4)** — polices hébergées avec le site : `src/styles/fonts.css` (Archivo, JetBrains Mono, Newsreader, sous-ensemble latin, fichiers identiques à ceux que Google servait, OFL 1.1), `preload` de Newsreader dans `index.html` / `personal-vault.html`, CSP sans `fonts.googleapis` / `fonts.gstatic` ; mesuré en local : accueil 20 → 19 requêtes, 4 → 0 distantes, `document.fonts.size` 42 → 3, Newsreader reçue avant le premier rendu grâce au `preload` (journal V7 § 4.1 bis) | V7 · commit C4 (D1, J1 fait le 16 septembre) ; `tests/fonts-contract.test.mjs`, `tests/presentation-derivatives.test.mjs`, `verify:v7` (`measure:surfaces --check` : 0 requête tierce hors App Check, 3 polices même origine sur l'accueil ; `measure:pf0` `fonts.woff2 = 3`) ; contrôle J2 local (journal V7 § 4.5, captures `scratchpad/v7/c4/captures/`) | à recetter après déploiement : onglet Réseau, cache vidé, 0 requête vers un hôte `fonts.*`, 3 `.woff2` même origine (`font/woff2`, `immutable`), `document.fonts.size === 3`, aucun décalage visible à la bascule, idem à 390 | à recetter après déploiement : `h1` en Newsreader (accueil, connexion), console sans rapport CSP ; Coffre personnel sous la CSP appliquée ; captures J2 comparées à la production | déploiement groupé C1-C5 + C4 (D16) ; le préchargement des deux autres polices et les sous-ensembles hors latin restent hors périmètre (dette au journal V7 § 4.4) |
| B6 | clos (requalifié : non imputable au site) | V0 · journal V0 § 4 et note sous le tableau B ; `scripts/lib/web-vitals-probe.mjs`, `tests/performance-api-hygiene.test.mjs` (dans `test:v7`) | à recetter : onglet neuf par page, console vide sur `/` et `/account/sign-in` | — | — |
| C1 | clos | V2 · seed v2 `--data-only` (six rappels, trois accès), `demoCartularyDocuments.ts:349,378` ; `demo-account`, `registry-aggregates` | à recetter (session démo) : Suivi six rappels dont un en retard, Accès trois | à recetter si R1 n'ouvre pas la démo | limite connue : rappel Submariner « en retard » après le 2026-12-15 |
| C2 | clos | V2 · `demoCartularyDocuments.ts:246-247,306` (`complete`, `lastVerifiedAt`) ; recette chiffrée non consignée en V3 | à recetter (session démo) : « À revoir 0 », cinq cartes « Complet » | idem si R1 n'ouvre pas la démo | — |
| C3 | clos | V2 puis V4 · `PublicationReadOnlySummary`, `PublicationSelectionTable` ; production V4 § 6 ; contrat l. 222, `publication-read-only-summary.test.tsx` | à recetter : page 05 démo, une table, 0 case ; à 390 : pas de défilement latéral de page | — | — |
| C4 | clos | V4 · `publication.ts:92-105`, note « Propriétaire, Transmission et Stockage restent privés » ; contrat V4 point 1 | à recetter : page 05 démo, note présente, aucune ligne personnelle | à recetter : page 05 éditeur, cellules « Non proposé » | — |
| C5 | clos | V2 · `RegistryApp.tsx:126` (entrée « Comparaison ») ; production V2 § 5 | à recetter (session démo) : « Comparaison » dans la barre latérale ; à 390 dans la barre 3 × 3 | — | — |
| D1 | clos | V5 · `App.tsx:670` (`canEdit`), `CartularyReadOnlyBlocks.tsx` ; V5 § 7 (0 contrôle sur les pages 00-04) ; contrat l. 617, 679 | à recetter : six pages démo sans contrôle ni crayon | à recetter : crayons par bloc, un bloc édité à la fois | bandeau hors session « Lecture seule · Connectez-vous… » à confirmer (D5 (a) de V5) |
| D2 | clos | V2 puis V5 · `AuditPanel.tsx:95,338`, `App.tsx:3411`, `CartularyTodoBoard.tsx:104` ; production V2 § 5 | à recetter : accueil démo sans message de synchronisation ; Preuves en français | — | — |
| D3 | clos | V6 · `useRevealActiveTab.ts`, `.dossier-bar` 69 px ; V6 § 6 (`#value` à 390 : onglet visible) ; contrat l. 896, `reveal-active-tab.test.tsx` | à recetter (390 émulé) : `#value` → onglet actif visible, ombres d'indice | à recetter : téléphone réel, paysage 844 × 390 (barre + onglets collants ≈ 32 % de la hauteur) | résidu : hauteur collante en paysage |
| D4 | clos | V6 · `.market-bars` en région défilante, `.comparable-group { min-width: 0 }` ; audit axe `scrollWidth ≤ clientWidth` (16 scènes × 2 fenêtres) | à recetter (390 émulé) : `#value` sans défilement latéral de page | à recetter : téléphone réel, paysage, 768 px | exception axe `aria-required-children` (`.sensitivity-table`) reportée V8 (D13) |
| D5 | clos | V6 · barre 3 × 3 (169 px), libellés, « Déconnexion » ; `registry-public-mobile-contract.test.mjs` ; mesuré sur réplique statique seulement (D9 de V6) | à recetter (session démo, 390 émulé) : barre 3 × 3 | à recetter : téléphone réel, libellés 9 px lisibles, un seul « Site d'accueil », en-tête compact | — |
| D6 | clos partiellement | V6 · bandeau non reproduit (`h1` à 174 px en production), onglets de la maquette défilants avec fondu ; `registry-public-mobile-contract.test.mjs` | à recetter (390 émulé) : hauteur avant le `h1`, onglets défilants | à recetter : téléphone réel avec barre d'adresse ; si ≈ 480 px réapparaît, rouvrir | — |
| D7 | clos partiellement (exemptions) | V6 · 44 px sur la barre du Cartulaire, l'accueil, les pages publiques, la barre du Registre ; cinq liens en phrase exemptés (WCAG 2.5.8) ; `touch-targets-contract.test.mjs` | à recetter (390 émulé) : cibles ≥ 44 px hors liens en phrase | à recetter : crayons, « Retirer », actions du popover À faire (36 px) en édition | résidus : popover À faire 36 px, actions d'édition non mesurées |
| D8 | clos | V6 · `h1` « Rolex Submariner », page 01 « Submariner » ; `cartulary-headings.test.mjs` ; V6 § 6 (un `h1` par page sur 13 scènes) | à recetter : un `h1` par page, « Rolex Submariner » | — | exception axe `heading-order` (tiroir Preuves) reportée V8 (D13) |
| D9 | clos (option « masquer la bascule ») | V6 · `DEFAULT_INTERFACE_LANGUAGE = 'FR'`, `lang="fr"` ; contrat l. 851, `barre-dossier-language-switch.test.tsx` | à recetter : aucune bascule, `document.documentElement.lang === 'fr'` | — | dette : ≈ 115 points (Cartulaire) et ≈ 1 100 chaînes (site) pour une version anglaise complète |
| D10 | clos | V6 · contrat, lecteur générique, test ; « mini -site » absent du bundle | à recetter : page 05, « Publiez un mini-site » | — | — |
