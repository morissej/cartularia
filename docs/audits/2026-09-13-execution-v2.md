...
## 5. Exécution (14 septembre)

**P0 — état d’entrée constaté** (navigateur intégré, sans compte, bundle `index-C454sD18.js` issu de `9d60495`) : `/watch-website?publicCode=DEMO-ROL-124060` → « Publication indisponible. Réessayer » ; page Publication de la Submariner en structure éditeur (« 14 contenus sélectionnés », « Tout décocher », cases à cocher) ; « Retour au Registre » vers `/registry` (V-A4 confirmé) ; aucune erreur « Collections indisponibles » avec ce bundle.

**Barrière rejouée par l’orchestrateur** après les deux compléments de couverture demandés par le relecteur du contrat (ordre de la garde de démonstration dans `handleReportPrint` ; tests de `loadPublicPublicationSummaries` : publié, révoqué, absent, refus de lecture, dérivation des statuts) : lint, `tsc`, vitest 62 fichiers / 275 tests, contrat 13 tests ; suite émulateur `npm run test:demo-account:full` au vert (seed complet, relance `--data-only --expect-no-writes` sans écriture, test émulateur). Commit `5d8bb18`.

**P4 — Hosting déployé** avant P1/P2 (le client lit l’état de publication à l’exécution, décision (d)) : bundle `index-BEoq1LpP.js`. Vérification anonyme : page Publication en rendu lecture (aucune case à cocher, une table, compteurs 14 / 14 / 20 / 23, « ce qui serait publié », aperçu local, bouton « Préparer le rapport PDF », aucun message de Collections) ; « Retour au Registre démo » vers `/registry/reg_cartularia_demo/items` ; panneau Preuves sans « Connexion requise », sans bouton, « Chaîne fictive de démonstration », lien « Voir les preuves du Registre démo », ni QR ni lien de mini-site avant publication ; `/community` → « Ouvrir le Registre démo » et « Retour à l’accueil » ; `/cartulary-demo` sans identifiant → Submariner, sans message de synchronisation ; `/registry/reg_cartularia_demo/items` sans session → écran de connexion avec « Ouvrir le Registre démo » ; `/watch-website?publicCode=DEMO-AP-15510ST` → « Publication absente ou révoquée. ». Console : seule une alerte CSP en mode rapport (iframe reCAPTCHA), sans effet.
