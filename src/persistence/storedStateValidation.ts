...
const KEY_NORMALIZERS: Record<string, Normalizer> = {
  ...
  'cartularia-market-depth': partialObjectValue({
    ...
    activeListings: (value, fallback) => value === null ? null : nonNegativeNumber(value, fallback),
    transactions12m: (value, fallback) => value === null ? null : nonNegativeNumber(value, fallback),
    medianDaysOnMarket: (value, fallback) => value === null ? null : nonNegativeNumber(value, fallback),
    lowValue: nonNegativeNumber,
    midValue: nonNegativeNumber,
    highValue: nonNegativeNumber,
  }),
  ...
  'cartularia-retained-valuation': objectValue({
    amount: nonNegativeNumber,
    saleCostAmount: nonNegativeNumber,
    taxAmount: nonNegativeNumber,
    explanation: stringValue,
    level: enumValue(['', 'owner_declared', 'ai_proposed', 'professional', 'transaction']),
    observedAt: stringValue,
    sourceLabel: stringValue,
    confidence: enumValue(['', 'low', 'medium', 'high']),
    currency: stringValue,
  }),
  'cartularia-insurance-coverages': arrayValue(objectValue({
    contractId: stringValue,
    carrierLabel: stringValue,
    contractReference: stringValue,
    insuredAmount: nonNegativeNumber,
    currency: stringValue,
    effectiveFrom: stringValue,
    effectiveTo: optionalString,
    basisLabel: stringValue,
    status: enumValue(['active', 'expired', 'pending']),
  })),
