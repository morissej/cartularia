...
const bridgeConfig = hasProductionConfiguration ? configuredBridge : emulatorMode ? {
...
if (bridgeProjectId && (bridgeProjectId === personalProjectId || bridgeProjectId === registryProjectId)) {
  throw new Error('La base de correspondance doit utiliser un troisième projet Firebase distinct.');
}

export const codeBridgeIsConfigured = Boolean(bridgeConfig);
