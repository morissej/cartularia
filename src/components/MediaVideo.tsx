...
export function MediaVideo({ asset, language = 'FR', ...props }: {
  ...
  const source = useMediaSource(asset, props.controls !== false && !needsRequest);
    ...
    return (
      <div className="media-load-prompt media-load-prompt--poster" role="img" aria-label={poster ? asset.name : language === 'FR' ? 'Aucune vignette disponible' : 'No thumbnail available'}>
        {poster
          ...
          : <p>{language === 'FR' ? 'Aucune vignette disponible · Ouvrez la vidéo pour la consulter.' : 'No thumbnail available · Open the video to view it.'}</p>}
      </div>
    );
  }
